...
def load_styles() -> None:
    st.markdown(
        """
        <style>
        .search-page-title { font-size: 1.9rem; font-weight: 700; color: #153e75; margin-bottom: 0.2rem; }
        .search-page-copy { font-size: 1rem; color: #526173; margin-bottom: 1.2rem; }
        .search-shell {
            border: 1px solid #d8e7f5;
            border-radius: 24px;
            padding: 20px 22px 8px 22px;
            background: radial-gradient(circle at top right, rgba(11, 116, 200, 0.08), transparent 30%), linear-gradient(180deg, #ffffff 0%, #f7fbff 100%);
            margin-bottom: 1rem;
        }
        .search-summary-card {
        ...
        .search-panel-title { font-size: 1.02rem; font-weight: 700; color: #23354d; margin-bottom: 0.15rem; }
        .search-panel-copy { font-size: 0.92rem; color: #6b7280; margin-bottom: 0.85rem; }
        .search-summary-label { font-size: 0.82rem; text-transform: uppercase; letter-spacing: 0.04em; color: #7a869a; margin-bottom: 0.2rem; }
        .search-summary-value { font-size: 1.08rem; font-weight: 600; color: #1f2937; }
        .service-chip {
        ...
        .service-chip-icon { margin-right: 0.3rem; }
        .service-chip-wrap { margin-top: 0.35rem; }
        .service-highlight {
        ...
        .service-highlight-value { font-size: 2rem; font-weight: 700; color: #0f172a; line-height: 1; margin-bottom: 0.25rem; }
        .service-highlight-copy { color: #64748b; font-size: 0.92rem; }
        .metric-card {
        ...
        .metric-label { font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.05em; color: #6b7280; margin-bottom: 0.45rem; }
        .metric-value { font-size: 1.18rem; font-weight: 700; color: #17212f; margin-bottom: 0.65rem; word-break: break-word; }
        .metric-track { width: 100%; height: 7px; border-radius: 999px; background: #edf2f7; overflow: hidden; }
        .metric-fill { height: 100%; border-radius: 999px; background: linear-gradient(90deg, #7aa7ff 0%, #2f6fed 100%); }
        .section-heading { font-size: 1.08rem; font-weight: 700; color: #20324b; margin: 0 0 0.85rem 0; }
        button[role="tab"] {
...
def render_customer_profile(profile: CustomerProfile) -> None:
    ...
    for col, (label, value) in zip((col1, col2, col3, col4), values):
        with col:
            st.markdown(
                f'<div class="search-summary-card"><div class="search-summary-label">{label}</div><div class="search-summary-value">{value}</div></div>',
                unsafe_allow_html=True,
            )
...
def _render_panel_header(title: str, copy: str) -> None:
    st.markdown(
        f'<div class="search-panel-title">{title}</div><div class="search-panel-copy">{copy}</div>',
        unsafe_allow_html=True,
    )
...
def _format_metric_value(value: object) -> str:
    ...
    if hasattr(is_missing, "all") and not isinstance(is_missing, bool):
        try:
            is_missing = bool(is_missing.all())
        except Exception:
            is_missing = False
    elif isinstance(is_missing, (list, tuple)):
    ...
    with right_col:
        with st.container(border=True):
            ...
            if not servicios_activos:
                ...
                st.markdown(
                    f'<div class="service-highlight"><div class="service-highlight-value">{len(servicios_activos)}</div><div class="service-highlight-copy">servicios activos encontrados para este cliente</div></div>',
                    unsafe_allow_html=True,
                )
                chips = "".join(
                    [
...
def render_contracts_summary(total_contracts: int, active_contracts: int) -> None:
    ...
    summary_items = [
        ("Contratos totales", total_contracts),
        ("Contratos activos", active_contracts),
    ]

    for start in range(0, len(summary_items), 4):
        current_items = summary_items[start:start + 4]
        cols = st.columns(4, gap="medium")
        for col, (label, value) in zip(cols, current_items):
            with col:
                st.markdown(
                    f'<div class="search-summary-card"><div class="search-summary-label">{label}</div><div class="search-summary-value">{value}</div></div>',
                    unsafe_allow_html=True,
...
def render_contracts_details(df_contracts: pd.DataFrame) -> None:
...
def render_contracts_section(df_contracts: pd.DataFrame) -> None:
    render_contracts_summary(len(df_contracts), 0)
    render_contracts_details(df_contracts)
